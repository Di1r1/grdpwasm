...
// SetAudioCapture requests audio input redirection to the server by setting
// the INFO_AUDIOCAPTURE flag in the Client Info PDU (MS-RDPBCGR
// TS_INFO_PACKET). Without it a Windows server never opens the AUDIO_INPUT
// dynamic virtual channel (MS-RDPEAI), no matter what the client supports.
// This corresponds to mstsc's "Record from this computer" option
// (audiocapturemode:i:1). Must be called before Login.
func (c *Client) SetAudioCapture(on bool) {
	if c.info == nil {
		return
	}
	if on {
		c.info.Flag |= INFO_AUDIOCAPTURE
	} else {
		c.info.Flag &^= INFO_AUDIOCAPTURE
	}
}
