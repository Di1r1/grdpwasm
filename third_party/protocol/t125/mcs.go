...
// SetExtraEarlyCapabilityFlags ORs additional flags into the Client Core
// Data earlyCapabilityFlags field. Used to probe which flags (if any) make
// a Windows server expose optional redirection channels.
func (c *MCSClient) SetExtraEarlyCapabilityFlags(mask uint16) {
	c.clientCoreData.EarlyCapabilityFlags |= mask
}
