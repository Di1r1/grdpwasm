...
type GfxHandler struct {
	...
	avc444Disabled bool
	// queueDepthHint is the user-configured minimum queueDepth reported in
	// FRAME_ACKNOWLEDGE PDUs.  A higher value makes the server believe the
	// client has a larger decode backlog, causing it to slow down or reduce
	// encoding quality.  0 means "report the real queue length" (no
	// throttling).  See SetQueueDepthHint.
	queueDepthHint atomic.Uint32
	// stallHint is the internal decoder-stall throttle (AVC/HW decoder
	// hiccups).  It is deliberately separate from queueDepthHint: the stall
	// code fires and clears it on every decoded frame, and sharing one value
	// silently wiped the user's setting (observed: "Quality" had no effect).
	// See SetStallHint.
	stallHint atomic.Uint32
...
// effectiveDepth — queueDepth, который уходит серверу: максимум из реальной
// длины очереди декодирования, пользовательской настройки и внутреннего
// throttling'а ступора декодера.
func (g *GfxHandler) effectiveDepth(real uint32) uint32 {
	if h := g.queueDepthHint.Load(); h > real {
		real = h
	}
	if h := g.stallHint.Load(); h > real {
		real = h
	}
	return real
}
...
func (g *GfxHandler) onEndFrame(data []byte) {
	...
	g.sendFrameAck(binary.LittleEndian.Uint32(data), g.effectiveDepth(uint32(len(g.decodeCh))))
}
...
// SetStallHint sets the internal decoder-stall throttle.  Kept apart from
// SetQueueDepthHint so that clearing the stall (on every healthy decoded
// frame) does not discard the user's configured throttle.
func (g *GfxHandler) SetStallHint(depth uint32) {
	g.stallHint.Store(depth)
}
