...
// expects to be present but that we don't process.
...
type RdpClient struct {
	...
	onAudioResetFn    func()
	onMicOpenFn       func(audin.MicFormat)
	onMicCloseFn      func()
	onMicProgressFn   func(string)
	onDvcReqFn        func(string)
	...
	// audinHandler is the active MS-RDPEAI handler; nil when not connected.
	audinHandler *audin.Handler

	// extraEarlyCaps are extra Client Core Data early capability flags used
	// for probing server behavior (set before Login).
	extraEarlyCaps uint16

	// micRequested asks the server for audio input redirection
	// (INFO_AUDIOCAPTURE in the Client Info PDU). Set before Login.
	micRequested bool

	// micPushCount counts PCM chunks handed to MicPush (diagnostics).
	micPushCount uint64

	// dvcClient owns dynamic virtual channels (client-initiated ones too).
...
func (g *RdpClient) doLogin(routingToken []byte) error {
	...
	if g.extraEarlyCaps != 0 {
		g.mcs.SetExtraEarlyCapabilityFlags(g.extraEarlyCaps)
	}
	g.sec = sec.NewClient(g.mcs)
	// Audio input redirection must be requested in the Client Info PDU
	// (INFO_AUDIOCAPTURE); otherwise the server never opens AUDIO_INPUT.
	g.sec.SetAudioCapture(g.micRequested)
	...
	// rdpdr (Device Redirection) — minimal MS-RDPEFS core handshake. Windows
	// withholds audio input redirection (AUDIO_INPUT) while this channel is
	// unanswered, so the announce + capability exchange must be completed.
	rdpdrBarrier := rdpdr.NewHandler()
	rdpdrBarrier.SetStageCallback(func(s string) {
		if g.onMicProgressFn != nil {
			g.onMicProgressFn(s)
		}
	})
	g.channels.Register(rdpdrBarrier)
	...
	// AUDIN (Audio Input / microphone) handler — static virtual channel
	// + DVC paths (MS-RDPEAI). Stays disarmed until SetMicEnabled(true).
	audinHandler := audin.NewHandler()
	audinHandler.SetOpenCallback(func(f audin.MicFormat) {
		if g.onMicOpenFn != nil {
			g.onMicOpenFn(f)
		}
	})
	audinHandler.SetCloseCallback(func() {
		if g.onMicCloseFn != nil {
			g.onMicCloseFn()
		}
	})
	audinHandler.SetProgressCallback(func(stage string) {
		if g.onMicProgressFn != nil {
			g.onMicProgressFn(stage)
		}
	})
	g.audinHandler = audinHandler
	g.channels.Register(audinHandler)
	...
	dvcClient.SetCreateReqObserver(func(name string) {
		if g.onDvcReqFn != nil {
			g.onDvcReqFn(name)
		}
	})
	// AUDIO_INPUT (microphone) is created by the CLIENT, not the server:
	// mstsc does the same when "Record from this computer" is enabled.
	// Without this the server never opens microphone redirection
	// (observed on Windows 11: no AUDIO_INPUT in the server's DVC list).
	g.dvcClient = dvcClient
	dvcClient.SetCreateRspCallback(func(name string, status uint32) {
		if name == audin.DvcName && status == 0 {
			g.audinReady.Store(true)
		}
		if g.onMicProgressFn != nil {
			if status == 0 {
				g.onMicProgressFn("rsp-ok")
			} else {
				g.onMicProgressFn("rsp-fail")
			}
		}
		slog.Debug("dvc: create rsp", "channel", name, "status", status)
	})
	dvcClient.SetReadyCallback(func() {
		if g.onMicProgressFn != nil {
			g.onMicProgressFn(fmt.Sprintf("dvccaps%d", dvcClient.NegotiatedVersion()))
		}
		// Клиентские create request для AUDIO_INPUT НЕ отправляем:
		// по MS-RDPEAI канал инициирует сервер (включается флагом
		// INFO_AUDIOCAPTURE), а клиентский запрос мог занять тот же
		// числовой id, что и канал сервера — тогда наши Data PDU уходили
		// не в тот канал и Windows сбрасывал сессию (connection reset).
	})
	dvcClient.SetUnknownCmdCallback(func(cmd uint8) {
		if g.onMicProgressFn != nil {
			g.onMicProgressFn(fmt.Sprintf("unk%d", cmd))
		}
	})
	...
	for _, name := range audin.DvcNames {
		dvcClient.RegisterHandler(name, audin.NewDvcAdapter(audinHandler))
	}
...
// MicFormat describes the PCM16 capture format agreed with the server.
type MicFormat = audin.MicFormat

// OnMicOpen registers a callback fired when the server opens microphone
// capture (or changes the format). The application should start pushing
// PCM16 frames via MicPush. Must be called before Login.
func (g *RdpClient) OnMicOpen(f func(MicFormat)) *RdpClient {
	g.onMicOpenFn = f
	return g
}

// OnMicClose registers a callback fired when microphone capture ends.
// Must be called before Login.
func (g *RdpClient) OnMicClose(f func()) *RdpClient {
	g.onMicCloseFn = f
	return g
}

// OnMicProgress reports microphone handshake milestones for diagnostics.
func (g *RdpClient) OnMicProgress(f func(string)) *RdpClient {
	g.onMicProgressFn = f
	return g
}

// OnDvcRequest reports (diagnostics) each DVC channel name the server
// asks to create, in arrival order.
func (g *RdpClient) OnDvcRequest(f func(string)) *RdpClient {
	g.onDvcReqFn = f
	return g
}

// SetMicRequested tells the server we want audio input redirection
// (INFO_AUDIOCAPTURE in the Client Info PDU, like mstsc's "Record from this
// computer"). Must be called before Login; a Windows server does not open
// the AUDIO_INPUT channel without it.
func (g *RdpClient) SetMicRequested(on bool) *RdpClient {
	g.micRequested = on
	return g
}

// SetExtraEarlyCapabilityFlags adds extra Client Core Data early capability
// flags (diagnostics: probing which flags make the server expose optional
// redirection channels). Must be called before Login.
func (g *RdpClient) SetExtraEarlyCapabilityFlags(mask uint16) *RdpClient {
	g.extraEarlyCaps = mask
	return g
}

// SetMicEnabled arms (true) or disarms (false) microphone capture.
// Disarmed: PushPCM frames are dropped. The server's OPEN is still accepted,
// so re-arming takes effect without reconnecting.
// Safe to call before Login (takes effect once connected).
func (g *RdpClient) SetMicEnabled(b bool) *RdpClient {
	if g.audinHandler != nil {
		g.audinHandler.SetEnabled(b)
	}
	return g
}

// MicPush sends one raw PCM16 (little-endian, interleaved) chunk to the
// server. Silently dropped unless capture is enabled and open.
func (g *RdpClient) MicPush(data []byte) {
	if g.audinHandler == nil {
		return
	}
	g.micPushCount++
	if g.onMicProgressFn != nil {
		if g.micPushCount == 1 {
			g.onMicProgressFn("push")
		} else if g.micPushCount == 100 {
			g.onMicProgressFn("pushmany")
		}
	}
	g.audinHandler.PushPCM(data)
}
